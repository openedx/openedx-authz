...
Decision item 4 called this an open problem to revisit later. ADR 0016 closes it. Authorization endpoints cannot compute or expose another domain's data, so ``PermissionValidationMeView``, ``RoleUserAPIView``, and ``RoleListView`` must not depend on the course-authoring flag, regardless of release timeline. Any future flag-aware behavior for these endpoints belongs to whichever domain ends up owning the flag.
