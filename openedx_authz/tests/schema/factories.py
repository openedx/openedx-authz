...
class InMemoryResource(DiscoveredResource):
    """A :class:`DiscoveredResource` that reads preset bytes instead of a file.

    Lets loader tests exercise parsing/digest logic without materializing files
    on disk. The ``load`` step reads bytes via ``DiscoveredResource.read_bytes``,
    so overriding just that method is enough to feed arbitrary content.
    """
